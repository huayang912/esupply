using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.LocalSystem.Entity.MasterData;

//TODO: Add other using statmens here.

namespace com.LocalSystem.Persistence.MasterData.NH
{
    public class NHSupplierBaseDao : NHDaoBase, ISupplierBaseDao
    {
        public NHSupplierBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateSupplier(Supplier entity)
        {
            Create(entity);
        }

        public virtual IList<Supplier> GetAllSupplier()
        {
            return GetAllSupplier(false);
        }

        public virtual IList<Supplier> GetAllSupplier(bool includeInactive)
        {
            string hql = @"from Supplier entity";
            if (!includeInactive)
            {
                hql += " where entity.IsActive = 1";
            }
            IList<Supplier> result = FindAllWithCustomQuery<Supplier>(hql);
            return result;
        }

        public virtual Supplier LoadSupplier(String code)
        {
            return FindById<Supplier>(code);
        }

        public virtual void UpdateSupplier(Supplier entity)
        {
            Update(entity);
        }

        public virtual void DeleteSupplier(String code)
        {
            string hql = @"from Supplier entity where entity.Code = ?";
            Delete(hql, new object[] { code }, new IType[] { NHibernateUtil.String });
        }

        public virtual void DeleteSupplier(Supplier entity)
        {
            Delete(entity);
        }

        public virtual void DeleteSupplier(IList<String> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from Supplier entity where entity.Code in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteSupplier(IList<Supplier> entityList)
        {
            IList<String> pkList = new List<String>();
            foreach (Supplier entity in entityList)
            {
                pkList.Add(entity.Code);
            }

            DeleteSupplier(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
