...
<script type="text/javascript">
	...
	function supplierSelectHandler(suppliers) {
		 DWRUtil.removeAllOptions("inboundLogs_inboundLog_supplierCode");
		 if (suppliers != null) {		 
			 DWRUtil.addOptions("inboundLogs_inboundLog_supplierCode",[{ name:'All', code:'-1' }], "code", "name");    
		 	DWRUtil.addOptions("inboundLogs_inboundLog_supplierCode",suppliers, "code", "name");    
		 }
	}
</script>
</head>
...
<s:form name="inboundLogForm" action="inboundLogs" method="post"
	...
	<table style="margin: 0px">
		<tr>
			...
			<td colspan="2"><s:select key="inboundLog.plantCode"
			...
			<td colspan="2"><s:select key="inboundLog.supplierCode"
...
<c:if test="${inboundLog != null}">
	<display:table name="paginatedList" cellspacing="0" cellpadding="0"
		requestURI="" id="inboundLogs" class="table" export="true">
		...
		<display:column property="memo" titleKey="inboundLog.memo" />
