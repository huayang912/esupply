...
<tr>
	<td><select name="<c:out value="${param.leftId}"/>"
		...
		<c:if test="${leftList != null}">
			<c:forEach var="list" items="${leftList}" varStatus="status">
				<option value="<c:out value="${list.value}"/>"><c:out
					value="${list.label}" escapeXml="false" /></option>
			</c:forEach>
		</c:if>
	</select></td>
	...
	<td><select name="<c:out value="${param.rightId}"/>"
		multiple="multiple" id="<c:out value="${param.rightId}"/>" size="5">
		<c:if test="${rightList != null}">			
			<c:forEach var="list" items="${rightList}" varStatus="status">		
				<c:set var="isHighlight" value="false" />
				<c:forEach var="hlist" items="${highightList}">
					<c:if test="${list.value == hlist.value}">
						<c:set var="isHighlight" value="true" />
					</c:if>
				</c:forEach>
				<c:if test="${isHighlight}">
					<option value="<c:out value="${list.value}"/>" class="flashcat3">
